...
<%	
	...
	try{
		...
		String quiz_idx 	= REQParam( params.get("quiz_idx") );

%>
<form name="aform" method="GET" action="quiz_detail.jsp">
...
<%		
		//////////////////////////////////// 등록 ///////////////////////////////////////
		if( "write".equals(cmd) ) {
			String sql = " INSERT INTO NARO_QUIZ_QUESTION SET quiz_idx=?, num=?, title=?, subtitle=?, contents=?, right_answer=?, del_yn='N', write_id=?, write_date=now() ";
			pstmt = con.prepareStatement( sql );
		
			paramcnt=1;	
			pstmt.setInt( paramcnt++, Integer.parseInt(quiz_idx) );
			pstmt.setString( paramcnt++, REQParam(params.get("num")) );
			...
			pstmt.setString( paramcnt++, REQParam(params.get("right_answer")) );
		...
		//////////////////////////////////// 수정 ///////////////////////////////////////
			...
			String sql = " UPDATE NARO_QUIZ_QUESTION SET num=?, title=?, subtitle=?, contents=?, right_answer=?, update_id=?, update_date=now() WHERE idx=? ";
			pstmt = con.prepareStatement( sql );
			...
			if( upd_cnt > 0 ) {
				out.println( "<script>alert('수정되었습니다.'); document.aform.action='quiz_detail_form.jsp'; document.aform.submit(); </script>" );
			} else {
				out.println( "<script>alert('수정되지 않았습니다. 다시 시도해주세요.'); document.aform.action='quiz_detail_form.jsp'; document.aform.submit(); </script>" );
			}			
		//////////////////////////////////// 삭제 ///////////////////////////////////////
		} else if( "remove".equals(cmd) ) {			
			String sql = " UPDATE NARO_QUIZ_QUESTION SET del_yn='Y', update_id=?, update_date=now() WHERE idx=? ";
			pstmt = con.prepareStatement( sql );			
